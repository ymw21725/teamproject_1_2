          ...
          scrollToX(originalScrollLeft, false);
			...
			function initialiseVerticalScroll()
			{
				if (isScrollableV) {
                    ...
                    //modifyscroll
                    var b = Math.ceil($(window).height() / 100) * 6;
                    if(b>78){ b=78; }
                    var con = 100 / b;
                    var tops = 0;
                    
                    for(var x=1; x < b+1; x++){
                        verticalTrack.append($('<div class="bar bar-'+x+'" bar='+x+' style="top:'+tops+'%"/>'));
                        tops = tops + con;
                    }

					verticalDrag.on("mouseenter",
						function()
						{
							...
							if (e.originalTarget === undefined || e.originalTarget == e.currentTarget) {
								var clickedTrack = $(this),
									...
									doScroll = function()
									{
                                        ...
                                        //modifyscroll
                                        barMoving(verticalDragPosition);
            ...
            function barMoving(pos){
                //modifyscroll
                var idx = 0; var lstIdx = 0;
                $('.bar').each(function(){
                    var vTop = $(this).css('top').substring(0, $(this).css('top').length - 2);
                    //console.log(parseFloat(vTop));
                    if(idx == undefined && parseFloat(pos) < 100){
                        idx = 0;
                    }
                    if(parseFloat(pos) < 100){
                        if(idx == 0 &&  parseFloat(pos+20) < parseFloat(vTop)){
                            idx = $(this).attr('bar');
                            $('.bar').removeClass('bar-active');
                        }
                    }else{
                        if(idx == 0 && parseFloat(pos*1.095) < parseFloat(vTop)){
                            idx = $(this).attr('bar');
                            $('.bar').removeClass('bar-active');
                        }
                    }
			...
			{   
                ...
                $('.bar').each(function(){
                    var vTop = $(this).css('top').substring(0, $(this).css('top').length - 2);
                    if(idx == undefined && verticalDragPosition < 50){
                        idx = 0;
                    }else if(idx == undefined && verticalDragPosition > 50){
                        idx = $(window).height()/15;
                    }
                    if(idx == 0 && parseFloat(verticalDragPosition*1.095) < parseFloat(vTop)){
                        idx = $(this).attr('bar');
                        $('.bar').removeClass('bar-active');
                    }
                });
